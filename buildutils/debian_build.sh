...
#
# [NOTICE][FIXME]
# This common script is modified for only fullock.
# Because fullock is repo name is "fullock", but package name has lib prefix.
...
#
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.preinst ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.preinst ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.preinst || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.preinst || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.postinst ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.postinst ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.postinst || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.postinst || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.prerm ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.prerm ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.prerm || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.prerm || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.postrm ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_NAME}.postrm ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.postrm || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_NAME}.postrm || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.preinst ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.preinst ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.preinst || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.preinst || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.postinst ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.postinst ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.postinst || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.postinst || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.prerm ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.prerm ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.prerm || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.prerm || exit 1
fi
if [ -f ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.postrm ]; then
	cp -p ${MYSCRIPTDIR}/lib${PACKAGE_DEV_NAME}.postrm ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.postrm || exit 1
	chmod +x ${EXPANDDIR}/debian/lib${PACKAGE_DEV_NAME}.postrm || exit 1
fi
...
#
ls ${BUILDDEBDIR}/lib${PACKAGE_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb >/dev/null 2>&1
if [ $? -eq 0 ]; then
	echo "===== show ${BUILDDEBDIR}/lib${PACKAGE_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb package: start ====="
	dpkg -c ${BUILDDEBDIR}/lib${PACKAGE_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb
	echo ""
	dpkg -I ${BUILDDEBDIR}/lib${PACKAGE_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb
	echo "===== show ${BUILDDEBDIR}/lib${PACKAGE_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb package: end ====="
fi

ls ${BUILDDEBDIR}/lib${PACKAGE_DEV_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb >/dev/null 2>&1
if [ $? -eq 0 ]; then
	echo "===== show ${BUILDDEBDIR}/lib${PACKAGE_DEV_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb package: start ====="
	dpkg -c ${BUILDDEBDIR}/lib${PACKAGE_DEV_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb
	echo ""
	dpkg -I ${BUILDDEBDIR}/lib${PACKAGE_DEV_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb
	echo "===== show ${BUILDDEBDIR}/lib${PACKAGE_DEV_NAME}_${PACKAGE_VERSION}-${BUILD_NUMBER}*.deb package: end ====="
fi
...
#
echo ""
echo "You can find lib${PACKAGE_NAME} ${PACKAGE_VERSION}-${BUILD_NUMBER} version debian package in ${BUILDDEBDIR} directory."
echo ""
exit 0
