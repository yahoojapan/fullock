...
#define	TMP_FILE_FORM					"/tmp/mptest-%d.dat"
...
static void Help(char* progname)
{
	...
	PRN("       %s -mutex(m) mutexname [mutex options]", progname ? programname(progname) : "program");
	...
	PRN(NULL);
	PRN("mutex options:");
	PRN("       -thread(t)   <thread count>    thread count for reader lock");
	PRN(NULL);
	PRN("other options:");
...
static bool MtReadWriteLock(PPROCPARAM pProcparam, PCHPARAM pParams)
{
	...
	for(int cnt = 0; cnt < pProcparam->rwl_params.loop_count; ++cnt){
		...
		// Lock
		while(true){
			if(pParams->is_reader){
				result = flobj.ReadLock(fd, 0, 1);
			}else{
				result = flobj.WriteLock(fd, 0, 1);
			}
			if(0 != result){
				ERR("Process(%d): Failed to %s lock by errno(%d).", getpid(), (pParams->is_reader ? "reader" : "writer"), result);
		...
		while(true){
			...
			if(0 != result){
				ERR("Process(%d): Failed to %s unlock by errno(%d).", getpid(), (pParams->is_reader ? "reader" : "writer"), result);
				pParams->failed_count++;
			}else{
				break;
			}
...
static bool MtMutexLock(PPROCPARAM pProcparam, PCHPARAM pParams)
{
	...
	// Loop
	for(int cnt = 0; cnt < pProcparam->mtx_params.loop_count; ++cnt){
		int	result;

		// Lock
		while(true){
			if(0 != (result = flobj.Lock(pProcparam->mtx_params.mutexname))){
				ERR("Process(%d): Failed to lock %s mutex by errno(%d).", getpid(), pProcparam->mtx_params.mutexname, result);
				pParams->failed_count++;
			}else{
				break;
			}
		}
		ISPRN("Process(%d): Locked mutex.", getpid());
		...
		// Unlock
		while(true){
			if(0 != (result = flobj.Unlock(pProcparam->mtx_params.mutexname))){
				ERR("Process(%d): Failed to unlock %s mutex by errno(%d).", getpid(), pProcparam->mtx_params.mutexname, result);
				pParams->failed_count++;
			}else{
				break;
			}
		}
		ISPRN("Process(%d): Unlocked mutex.", getpid());
...
int main(int argc, char** argv)
{
	...
	if(st_procparam->common.is_rwlock){
		...
		fprintf(stdout, "thread count:                 %d\n", st_procparam->mtx_params.thread_count);
		fprintf(stdout, "loop count for each thread:   %d\n", st_procparam->mtx_params.loop_count);
		fprintf(stdout, "total count:                  %d ( = %d * %d)\n", st_procparam->mtx_params.thread_count * st_procparam->mtx_params.loop_count, st_procparam->mtx_params.thread_count, st_procparam->mtx_params.loop_count);
	}
	fprintf(stdout, "---------------------------------------------------------------------\n");
