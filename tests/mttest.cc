...
static bool MtReadWriteLock(PTHPARAM pParams)
{
	...
	for(int cnt = 0; cnt < procparam.rwl_params.loop_count; ++cnt){
		...
		while(true){
			...
			if(0 != result){
				ERR("Thread(%d): Failed to %s lock by errno(%d).", tid, (pParams->is_reader ? "reader" : "writer"), result);
		...
		while(true){
			...
			if(0 != result){
				ERR("Thread(%d): Failed to %s unlock by errno(%d).", tid, (pParams->is_reader ? "reader" : "writer"), result);
				pParams->failed_count++;
			}else{
				break;
			}
...
static bool MtMutexLock(PTHPARAM pParams)
{
	...
	// Loop
	for(int cnt = 0; cnt < procparam.mtx_params.loop_count; ++cnt){
		int	result;

		// Lock
		while(true){
			if(0 != (result = flobj.Lock(procparam.mtx_params.mutexname))){
				ERR("Thread(%d): Failed to lock %s mutex by errno(%d).", tid, procparam.mtx_params.mutexname, result);
				pParams->failed_count++;
			}else{
				break;
			}
		}
		ISPRN("Thread(%d): Locked mutex.", tid);
		...
		// Unlock
		while(true){
			if(0 != (result = flobj.Unlock(procparam.mtx_params.mutexname))){
				ERR("Thread(%d): Failed to unlock %s mutex by errno(%d).", tid, procparam.mtx_params.mutexname, result);
				pParams->failed_count++;
			}else{
				break;
			}
		}
		ISPRN("Thread(%d): Unlocked mutex.", tid);
...
int main(int argc, char** argv)
{
	...
	// display result
	fprintf(stdout, "=====================================================================\n");
	fprintf(stdout, "test type:                    %s\n", procparam.common.is_rwlock ? "rwlock" : "mutex");
	if(procparam.common.is_rwlock){
		...
		fprintf(stdout, "thread count:                 %d\n", procparam.mtx_params.thread_count);
		fprintf(stdout, "loop count for each thread:   %d\n", procparam.mtx_params.loop_count);
		fprintf(stdout, "total count:                  %d ( = %d * %d)\n", procparam.mtx_params.thread_count * procparam.mtx_params.loop_count, procparam.mtx_params.thread_count, procparam.mtx_params.loop_count);
	}
	fprintf(stdout, "---------------------------------------------------------------------\n");
